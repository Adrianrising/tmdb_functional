...
class RelatedMoviesSectionView extends StatefulWidget {
  const RelatedMoviesSectionView({Key? key, required this.movieId})
      : super(key: key);
  final int movieId;

  @override
  State<RelatedMoviesSectionView> createState() =>
      _RelatedMoviesSectionViewState();
}
...
class _RelatedMoviesSectionViewState extends State<RelatedMoviesSectionView> {
  ...
  void initState() {
    ...
    // tmdbApply
    //     .getSimilarMoviesFromBoxAsStream(widget.movieId, page)
    //     .listen((event) {
    //   setState(() {
    //     similarMovies = event ?? [];
    //   });
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.atEdge) {
        if (_scrollController.position.pixels != 0) {
          page++;
          // tmdbApply.getSimilarMoviesFromBoxAsStream(widget.movieId, page).listen((event) {
          //   final temp=event??[];
          //   if(temp.isNotEmpty){
